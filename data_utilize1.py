...
def convert_to_one_hot(seg):
    # vals = np.unique(seg)
    res = np.zeros([16] + list(seg.shape), seg.dtype)
...
class Data(Dataset.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # a = self.im_3D[index][14:]
        # print(a)

        label = sitk.ReadImage(Path_lab + self.im_3D[index][14:])
        ...
        # image3 = read_image(image3, spacing_target, spacing_target)
        # label = read_label(label, spacing_target, spacing_target)
        ...
        image = resize(image3, (d// beishu, w // beishu, h // beishu), order=3, mode='edge')
        image3 = image.astype(np.float32)  # [d,w,h]

        tem_label = resultlabel
        vals_label = np.unique(label)
        result_vals_label = np.zeros((16,d// beishu, w// beishu, h// beishu ))
        for i in range(16):
            result_vals_label[i,:,:,:] = resize(tem_label[i].astype(float), (d// beishu, w // beishu, h // beishu), order=1, mode='edge')[None]
        # label = vals_label[np.vstack(result_vals_label).argmax(0)]
        label = result_vals_label
